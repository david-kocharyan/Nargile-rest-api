...
class Community_Api extends REST_Controller
{
	...
	private function get_upcoming($res)
	{
		...
		$sql = ("SELECT users.id as id, concat('/plugins/images/Logo/', users.image) as image, username, first_name, last_name, DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(0), interval date_of_birth second),'%M %D %YY') as date_of_birth
				FROM friends 
				JOIN  users on users.id = friends.from_id 
				...
				UNION
				(SELECT users.id as id, concat('/plugins/images/Logo/', users.image) as image, username, first_name, last_name, DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(0), interval date_of_birth second),'%M %D %YY') as date_of_birth
				FROM friends 
				JOIN  users on users.id = friends.to_id 
