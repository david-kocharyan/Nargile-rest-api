...
class Restaurants_Api extends REST_Controller
{
    ...
    {
        $this->db->select("restaurants.name as restaurant, area.name as area, concat('/plugins/images/Restaurants/', restaurants.logo) as logo,");
        $this->join();
        $this->where();
        $this->db->like('restaurants.name', 'match', 'after');
        $this->db->limit(10);
        $this->db->order_by("restaurants.name");
        $data = $this->db->get("restaurants");
        return $data != null ? $data : "";
    }
