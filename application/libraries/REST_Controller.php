...
//	JWT TOKEN PART
	protected function verify_get_request()
	{
		...
		if(isset($headers['Authorize'])) {
			...
			$response = ['status' => $status, 'msg' => ' Provide Authorize token!! '];
			return $response;
		}
		// Use try-catch
		...
		try {
			...
			if ($data === false) {
				$status = self::HTTP_UNAUTHORIZED;
				$response = ['status' => $status, 'msg' => 'Wrong Token Provided!'];
				return $response;
			} else {
				$this->db->select("*");
				$this->db->from("tokens");
				...
				if(null == $data || $data->time < time()) {
//                    if($data != null && $data->time < time()) {
//                        $this->db->delete('tokens', ['user_id' => $data->user_id] );
//                    }
					$status = self::HTTP_UNAUTHORIZED;
					$response = ['status' => $status, 'msg' => 'Expired Token Time!'];
					return $response;
				}
				return $data->user_id;
			}
...
//    Simple function for generating API token
